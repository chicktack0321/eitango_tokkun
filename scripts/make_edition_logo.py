...
**全級（2級・準2級・3級・4級・準1級・1級）が手描きの素材に差し替え済み**
（2026-09-12 / 準1級・1級は 2026-09-20）。このスクリプトが作るのはWindows標準
フォントによる近似で、書体が本物と違う。既にある素材は `--force` を付けない限り
上書きしない。

TOEIC 版など新しいエディションを作るときの出発点として残してある。出力した画像を
...
# 値は手描き素材の左端上下の実測。目分量で近い色を置くと、描き直したときに
# ホーム画面で「同じ級なのに違う色」に見える。
PALETTE = {
    "GP1": ((204, 0, 126), (255, 31, 169)),   # マゼンタ。赤橙（準2級）より赤紫寄り
    "G1": ((107, 31, 173), (188, 63, 222)),   # 紫。紺（2級）とは明度も色相も離してある
}
