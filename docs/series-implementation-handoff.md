...
>
> **アイコンは `scripts/make_edition_logo.py --edition GP2` で機械的に描き起こしている。**
> 配色と背景グラデーションは2級の元画像から実測した値をそのまま使うので、ホーム画面に
> 並べたときに同じシリーズに見える。級を増やすたびに手で作ると字面の位置と青が少しずつ
> ずれるため。2級の元画像は公開済みなので生成対象から外してある。
> フォントは Windows 標準の BIZ UDゴシック B で、2級の手作業版とは書体が違うが、
> **このまま使う**（2026-09-12 に確認済み）。差し替えたくなったら元画像を置き換えて
> `make_app_icon.py --edition GP2` を流し直すだけでよい。
