...
vocab/g4_basic_words.txt       下位級の「既習」選定リスト
vocab/g4_core_words.txt        下位級の「学習対象」選定リスト
vocab/g4_bridge_words.txt      生成物（レビュー用）。手で編集しない
vocab/jhs_band_examples.txt    「word | 例文」。editions.G4/G3.example になる
vocab/gp1_bridge_words.txt     G2 core からの選抜（準1級の bridge 母体）
vocab/gp1_new_core.txt         準1級専用の新規 core 語（全フィールド）
vocab/gp1_new_bridge.txt       準1級専用の新規語のうち無料帯に置くもの（同上）
vocab/g1_bridge_words.txt      GP1 core からの選抜（1級の bridge 母体）
vocab/g1_new_core.txt          1級専用の新規 core 語（全フィールド）
...
| GP2（準2級） | 審査提出済み（core 独自性 54.5%） | 886 | 1,257 | 1,182 | 3,325 |
| G3（3級） | 語彙設計 完了・**課金なし**（§4 無料エディション） | 297 | 589 | 800 | 1,686 |
| G4（4級） | 語彙設計 完了・**課金なし**（同上） | 297 | 386 | 297 | 980 |
| GP1（準1級） | 語彙設計 完了（core 独自性 72.6%） | 2,681 | 697 | 1,491 | 4,869 |
| G1（1級） | 語彙設計 完了（core 独自性 100%） | 3,378 | 409 | 1,219 | 5,006 |
| TOEIC | 未着手 | — | — | — | — |

マスターの総語数は 6,732（G2 由来 3,955 + GP2 専用 644 + GP1 専用 914 + G1 専用 1,219）。
G4・G3 は新規語を足していない（母体は G2 の基礎1,512語）。
...
```jsonc
{
  ...
  "seedVersion": 22,     // 生成物に載る版。§8 の規則で上げる
...
| --- | --- | --- | --- |
| G4 | 4級帯（A1）203語 | G2 basic の886語 | **完了（無料アプリ）**: 例文589件を新規執筆 |
| G3 | 3級帯（A1+〜A2）626語 | `gp2_bridge_words.txt` の626語 | **完了（無料アプリ）**: 例文は GP2 用を流用 |
| GP2 | 準2級帯（A2）1,182語 | G2 bridge の category A（538語）のみ | **完了**: 新規 core 644語 + bridge 626語の例文 |
| G2 | 2級帯（B1）1,274語 | — | **変更不可**（公開済み） |
| GP1 | 準1級帯（B2）1,491語 | bridge = G2 core 1,274語から529語を選抜。**非選抜の745語は core に据える**（下記） | **完了**: 新規914語（core 746 + bridge 168） |
| G1 | 1級帯（C1）1,219語 | bridge = GP1 の**新規** core から409語を選抜 | **完了**: core 1,219語をすべて新規 |
...
実測（2026-09-20）: G2 58.5% / GP2 54.5% / GP1 72.6% / G1 100.0%。
G2 が 100% から下がったのは、準1級の bridge へ529語を選抜スライドさせたため（設計どおり）。
...
目安: GP1 bridge = G2 core から約550語（43%）。**実績: 529語（41.5%）。**
...
### GP1 / G1 で実際にやったこと（2026-09-20）

上位級は下位級と事情が逆で、**その級固有の語を新規に書ける**ため独自性は達成しやすい。
難しいのは「どこまでを既習として無料で見せるか」の線引きだった。

| | core | 独自性 | 露出させたもの |
| --- | --- | --- | --- |
| GP1 | 再利用 745 + 新規 746 = **1,491** | **72.6%** | G2 core の41.5%（529語）を bridge へ |
| G1 | 新規 **1,219**（全語1級専用） | **100.0%** | GP1 の新規 core の409語を bridge へ |

決めたことを3つ残す。

1. **G2 core の非選抜745語を GP1 の core に据えた。** スライド原則（級Nの core は級N+1で
   bridge の母体）をそのまま当てると、選抜から漏れた745語は GP1 に載らない。しかし中身は
   `abstract` `advocate` `ambiguous` `arbitrary` のような実質B2帯の語で、準1級アプリから
   丸ごと抜け落ちる方が商品として不自然だった。**有料帯どうしの重複は独自性ルールに
   抵触しない**（ルールAが見るのは他エディションの*無料帯*）ので、core に置いた。
   例文は G2 core が100%持っているため、追加執筆は発生していない
2. **G1 の bridge は「GP1 の新規 core」からだけ選んだ。** GP1 core の半分は G2 core 由来で、
   それを1級の無料帯に流すと **2級アプリの独自性が 58.5% → 0%** になる。
   `apply_g1_placement.py` はこれを検査し、選定リストに G2 core 由来の語が混じっていたら
   その場で止める（独自性検査より手前で、原因が分かる形で落とすため）
3. **GP1 の新規語のうち頻出度A・副詞・句動詞（168語）は bridge に置いた。** 新規914語を
   全部 core にすると 1,659語となり、§3 の上限1,500語を超える。「完走できる分量」を
   保つことを優先し、2級帯のすぐ上にあたる語を無料帯へ降ろした

残る非搭載: GP1 の新規 core の頻出度C（337語）は G1 に載せていない。準1級アプリの
売り物を守るため。1級アプリの basic は2級までの既習語（3,378語）で足りている。

### 無料エディション（G4 / G3）

**4級・3級はコア独自性ルールを満たせない。有料にせず無料アプリとして出す**（2026-09-12 決定）。

理由は語彙の分布にある。4級・3級の語彙帯は、公開中の2級アプリの無料 basic 帯
（1,512語）に構造的にほぼ全部含まれている。実測すると:

| サンプル | 語数 | master に無い語 |
| --- | --- | --- |
| 中学レベルの基本語 | 143 | 5（3.5%） |
| やや頻度の低い中学語 | 148 | 15（10%） |

つまり「4級・3級固有で、2級アプリに無い語」がほとんど存在しない。独自性50%を満たすには
両級で合わせて1,200語ほどの新規語が要るが、その帯に実在する語が尽きているため、
**級より上の語を core に入れて水増しする**ことになり、級別アプリとしての正しさが崩れる。
売り物を守るために商品の中身を壊すのは本末転倒なので、課金しない方を選んだ。

実装上の扱い:

- `EditionSpec.unlockProductID` が `nil` のエディションは課金なし。StoreKit に触れず、
  購入導線（ホームのカード・クイズのロック案内・「購入を復元」）を出さず、
  全語彙を最初から出題対象にする（`AccessRights.isFreeEdition`）
- `check_core_exclusivity.py` の `FREE_EDITIONS` に列挙する。独自性は評価しないが、
  **その core は他エディションから見れば無料帯**なので、free 集合には数える
  （無料アプリが他級の課金対象を丸ごと配ってしまう事故を検出するため）
- 3層構造（basic / bridge / core）は残す。無料エディションでは課金の境界ではなくなり、
  「既習かどうか」と「既定の出題範囲」を表すだけになる

この判断は下位級に限る。**準1級・1級で同じことをしてはならない**。上位級は core に
当該級固有の語を新規に書けるため（§3 の表）、独自性は達成可能であり、
達成しないまま無料にすると単に収益を捨てることになる。
...
| `scripts/apply_gp2_placement.py` | 選定リスト + 母体 tier から配置を計算し、専用の新規語と例文 override も含めて master へ書き戻す | 冪等。何度でも可 |
| `scripts/apply_lower_grades_placement.py` | G4 と G3 の配置。下位2級は同じ母体を帯の切り方だけ変えて使うので1本にまとめてある | 冪等。何度でも可 |
| `scripts/apply_gp1_placement.py` | GP1 の配置。G2 core の選抜と、準1級専用の新規語（core / bridge）を master へ書き戻す | 冪等。何度でも可 |
| `scripts/apply_g1_placement.py` | G1 の配置。**GP1 のあとに流す**（GP1 の配置を入力にするため） | 冪等。何度でも可 |
...
- **語が変わっていないエディションの seed は作り直さない。** 生成し直すと version だけが
  上がった差分になり、公開中アプリに無意味な再 Upsert を配ることになる。実際、同梱 seed の
  version はエディションごとにずれている（G2=18 / GP2・G3・G4=21 / GP1・G1=22）。
  `build_seed.py --check` は `words` だけを比較するので、この差では落ちない
- エディションごとに分けず、master 全体で1つの連番にする（現在 22）
...
python scripts/apply_gp1_placement.py
python scripts/apply_g1_placement.py      # GP1 のあとに流すこと
