...
vocab/<id>_*_words.txt     選定リスト。人間がレビューする中間成果物（§9）
        ↓ scripts/apply_<id>_placement.py   （配置を計算して master へ書き戻す。冪等）
...
| GP2（準2級） | 配置済み・**§4の是正が必要** | 886 | 626 | 1,169 | 2,681 |
...
| GP2 | 準2級帯（A2） | G2 bridge から**約800語を選抜** | **新規 core 約400語** + bridge 626語の例文（§4 是正） |
...
実測（`python scripts/check_core_exclusivity.py`、2026-08-15）:

```
[OK] G2:  core 1274語 / 独自 1274語 = 独自性 100.0%（目標 50%）
[NG] GP2: core 1169語 / 独自    0語 = 独自性   0.0%（目標 30%）
       G2 の無料帯に 1169語 (100.0%) が露出
```

GP2 の core は G2 の bridge 全量なので、**準2級アプリの ¥500 の対象が、2級アプリでは
1円も払わずに出題される**。放置できない。
...
| エディション | 目標 | 根拠 |
| --- | --- | --- |
| 既定（新規設計する級） | **50%** | core の過半がその級固有なら、他アプリは代替にならない |
| GP2 | **30%** | G2 が公開済みで変更できず、準2級帯の語の多くは既に G2 の無料 bridge にある。構造的に 100% は不可能。到達可能な線を置く（下の算数） |
| G2 | 50%（現状 100%） | 公開済み。下位級側の配置で守る |
...
core を「G2 bridge 全量」から次に組み替える。

| | 語数 | 独自性 |
| --- | --- | --- |
| 現状 | 再利用 1,169 + 新規 0 = 1,169 | 0.0% |
| **是正後（推奨）** | 再利用 **800** + 新規 **400** = **1,200** | **33.3%** ✔ |
| 参考: 再利用を減らさず新規追加だけで達成する場合 | 1,169 + 501 = 1,670 | 30.0%（core 語数の上限1,500を超える。不採用） |

- 再利用から外す約369語は **GP2 の bridge へ降ろす**。G2 bridge は例文カバレッジ100%なので
  例文の追加執筆は発生せず、bridge は 626 → 約995語になる
- 新規 core 約400語は、準2級帯（A2）で G2 の3,955語に含まれない語を新設する。
  日常・学校・買い物・旅行の具体語や基本句動詞に候補が多い
- 併せて、下位級向けに訳・例文が硬い語は `editions.GP2` の override で平易化する（§5）

正直に書いておくと、**隣接級である以上「準2級帯の語が2級アプリの無料帯にある」こと自体は
ゼロにできない**（G2 を変更できないため）。是正の狙いは、準2級アプリの core の3分の1を
その級専用にし、さらに級相応の訳・例文で差別化して、2級アプリが準2級アプリの代替に
ならないようにすることにある。
...
| `scripts/apply_<id>_placement.py` | 選定リスト + 母体 tier から配置を計算し master へ書き戻す | 冪等。何度でも可 |
