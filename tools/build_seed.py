#!/usr/bin/env python3
"""【旧方式・非推奨】data/ のテキストから seed を組み立てる、マスター導入前のスクリプト。

語彙の正本は `vocab/master.json` に移った（docs/vocab-database-spec.md）。
同梱 seed の生成は `python scripts/build_seed.py --edition <ID>` を使うこと。
こちらを実行しても出力先は現在アプリが同梱しているパスではないため、アプリには反映されない。
過去の入力テキストを読み直すときの参照用として残してある。
